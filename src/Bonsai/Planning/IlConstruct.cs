namespace Bonsai.Planning
{
    ...
    public class IlConstruct : IConstruct
    {
        ...
        public CreateInstance Create(RegistrationContext context, IEnumerable<Contract> contracts)
        {
            ...
            var methodName = $"{context?.ImplementedType?.FullName?.Replace(".", "_")}_ctor_{_count}";
            _count++;
            ...
            foreach (var param in ctor.Parameters)
            {
                ...
                {
                    position = createInstancesCache.Count;
        ...
        private static void LoadArg(ILGenerator il, int position)
        {
            switch (position)
            {
        ...
        private static void LoadIndex(ILGenerator il, int position)
        {
